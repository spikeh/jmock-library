...
public class PaperTweeterTableNode extends Node<TweeterCustomer> {
    private Map<Integer, Delay> delayMap = new HashMap<>();
    private PaperTweeterModel tweeterModel;
    private String tableName;
    private String testName;

    public PaperTweeterTableNode(PaperTweeterModel network, Sim sim, String tableName, String testName) {
    ...
    private class EndServiceEvent extends Event<TweeterCustomer> {
        ...
        public boolean invoke() {
            ...
            if (delay == null) {
                try {
                    InputStreamReader isr = new InputStreamReader(getClass().getResourceAsStream(String.format("/paper/%s_%s.txt", testName, tableName)));
                    try (BufferedReader br = new BufferedReader(isr)) {
                        Distribution alias = new Alias(br);
