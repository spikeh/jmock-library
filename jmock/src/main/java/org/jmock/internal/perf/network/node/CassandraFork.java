...
import static org.jmock.internal.perf.network.request.CassandraRequestType.READ_LOCAL;
import static org.jmock.internal.perf.network.request.CassandraRequestType.READ_REMOTE_ID;
...
public class CassandraFork extends CassandraNode {
    ...
    private void sendReadRemoteIdCustomers(CassandraCustomer customer, int numNodes) {
        ...
        while (sent.size() < numNodes) {
            ...
            sent.add(dest);
            CassandraCustomer newCustomer = new CassandraCustomer(network, sim, customer.threadId(), customer.invocation(), READ_REMOTE_ID, customer.coordinatorNodeId());
